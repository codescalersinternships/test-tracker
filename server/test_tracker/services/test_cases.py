...
def month_filter_test_cases_based_on_test_suites(test_suites: TestSuites, month: str) -> TestCases:
    ...
    for case in test_cases:
        ...
        if case_type is not None:
            response[case.modified.day] = {
                "total": len(test_cases.filter(modified__day__gte=case.modified.day, run=True)),
                "passed": len(passed.filter(modified__day__gte=case.modified.day, run=True)),
                "failed": len(failed.filter(modified__day__gte=case.modified.day, run=True)),
